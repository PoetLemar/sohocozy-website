...
Resolution observed at 04:04:59 UTC: both purchased-domain HTTPS pages and JavaScript returned 200, the apex certificate became active, and the watcher exited successfully. The provider completed provisioning while the restored configuration stayed unchanged. The prepared provider support draft was never sent and is no longer needed for this incident.

Final verification passed at 04:10:24 UTC. Deployment `5e381e2f-f1c8-4438-adb7-00f0f5e397b7` serves source `c949fd1`; both purchased-domain pages, matching JavaScript, video, poster, and favicon return 200 with verified TLS. The starter hostname redirects to the apex with HTTP 302 and preserves paths and query strings. Desktop and mobile browser checks passed on both domains, including video playback, knob interaction, resource loading, and mobile width. No signup was submitted; email delivery is outside these domain-readiness checks. The private outcome receipt is `production-final-20260908.json`. This closes the domain-activation investigation; retain the loop for future authorized storefront work.
